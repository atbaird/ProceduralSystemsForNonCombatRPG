...
float Compute2dFractalNoise(float posX, float posY, float scale, unsigned int numOctaves, float octavePersistence, float octaveScale, bool renormalize, unsigned int seed)
{
	...
	Vector2 currentPos = Vector2(posX * invScale, posY * invScale);

	for (unsigned int octaveNum = 0; octaveNum < numOctaves; ++octaveNum)
	{
		// Determine noise values at nearby integer "grid point" positions
		Vector2 cellMins = Vector2((float)FastFloor(currentPos.x), (float)FastFloor(currentPos.y));
...
float Compute2dPerlinNoise(float posX, float posY, float scale, unsigned int numOctaves, float octavePersistence, float octaveScale, bool renormalize, unsigned int seed)
{
	...
	for (unsigned int octaveNum = 0; octaveNum < numOctaves; ++octaveNum)
	{
		...
		Vector2 cellMins = Vector2((float)FastFloor(currentPos.x), (float)FastFloor(currentPos.y));
		Vector2 cellMaxs = Vector2(cellMins.x + 1.f, cellMins.y + 1.f);
		int indexWestX = (int)cellMins.x;
		int indexSouthY = (int)cellMins.y;
		int indexEastX = indexWestX + 1;
		int indexNorthY = indexSouthY + 1;
		...
		float dotSouthWest = gradientSW.dotProduct(displacementFromSW);
		float dotSouthEast = gradientSE.dotProduct(displacementFromSE);
		float dotNorthWest = gradientNW.dotProduct(displacementFromNW);
		float dotNorthEast = gradientNE.dotProduct(displacementFromNE);
...
float Compute3dPerlinNoise(float posX, float posY, float posZ, float scale, unsigned int numOctaves, float octavePersistence, float octaveScale, bool renormalize, unsigned int seed)
{
	...
	for (unsigned int octaveNum = 0; octaveNum < numOctaves; ++octaveNum)
	{
		...
		float dotBelowSW = gradientBelowSW.dotProduct(displacementFromBelowSW);
		float dotBelowSE = gradientBelowSE.dotProduct(displacementFromBelowSE);
		float dotBelowNW = gradientBelowNW.dotProduct(displacementFromBelowNW);
		float dotBelowNE = gradientBelowNE.dotProduct(displacementFromBelowNE);
		float dotAboveSW = gradientAboveSW.dotProduct(displacementFromAboveSW);
		float dotAboveSE = gradientAboveSE.dotProduct(displacementFromAboveSE);
		float dotAboveNW = gradientAboveNW.dotProduct(displacementFromAboveNW);
		float dotAboveNE = gradientAboveNE.dotProduct(displacementFromAboveNE);
...
float Compute4dPerlinNoise(float posX, float posY, float posZ, float posT, float scale, unsigned int numOctaves, float octavePersistence, float octaveScale, bool renormalize, unsigned int seed)
{
	...
	for (unsigned int octaveNum = 0; octaveNum < numOctaves; ++octaveNum)
	{
		...
		float dotBeforeBSW = gradientBeforeBSW.dotProduct(displacementFromBeforeBSW);
		float dotBeforeBSE = gradientBeforeBSE.dotProduct(displacementFromBeforeBSE);
		float dotBeforeBNW = gradientBeforeBNW.dotProduct(displacementFromBeforeBNW);
		float dotBeforeBNE = gradientBeforeBNE.dotProduct(displacementFromBeforeBNE);
		float dotBeforeASW = gradientBeforeASW.dotProduct(displacementFromBeforeASW);
		float dotBeforeASE = gradientBeforeASE.dotProduct(displacementFromBeforeASE);
		float dotBeforeANW = gradientBeforeANW.dotProduct(displacementFromBeforeANW);
		float dotBeforeANE = gradientBeforeANE.dotProduct(displacementFromBeforeANE);
		float dotAfterBSW = gradientAfterBSW.dotProduct(displacementFromAfterBSW);
		float dotAfterBSE = gradientAfterBSE.dotProduct(displacementFromAfterBSE);
		float dotAfterBNW = gradientAfterBNW.dotProduct(displacementFromAfterBNW);
		float dotAfterBNE = gradientAfterBNE.dotProduct(displacementFromAfterBNE);
		float dotAfterASW = gradientAfterASW.dotProduct(displacementFromAfterASW);
		float dotAfterASE = gradientAfterASE.dotProduct(displacementFromAfterASE);
		float dotAfterANW = gradientAfterANW.dotProduct(displacementFromAfterANW);
		float dotAfterANE = gradientAfterANE.dotProduct(displacementFromAfterANE);
