...
#if defined( DISABLE_ASSERTS )
#define ASSERT_RECOVERABLE_NO_POPUP( condition, errorMessageText ) { (void)( condition ); }
#else
#define ASSERT_RECOVERABLE_NO_POPUP( condition, errorMessageText )											\
{																									\
	if( !(condition) )																				\
	{																								\
		const char* conditionText = #condition;														\
		RecoverableWarningNoPopUp( __FILE__,  __FUNCTION__, __LINE__, errorMessageText, conditionText );	\
	}																								\
}
#endif