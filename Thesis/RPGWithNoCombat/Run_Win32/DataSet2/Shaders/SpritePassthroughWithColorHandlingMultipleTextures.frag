...
const int gNumberOfTextures = 10;
uniform sampler2D gDiffuseTex[gNumberOfTextures];
uniform int gColorChoice = 0;
in vec2 passUV;
in vec4 passColor;
flat uint passSpriteIdx = 0;
...
{
	int maxColChoice = 6;
	vec4 c = texture(gDiffuseTex[passSpriteIdx], passUV);
	float luminance = GetLuminance(c);
	vec3 defaul = passColor.rgb * luminance;
	vec3 vec3EndCol = c.rgb * defaul;
